...
/*******************************************************************************************************
    ...
    Description:
    ============
    Writes a new duty cycle to the pwm_id specified, based on the degree provided. The duty cycle is equal 
    to the value needed to move the servo by the provided degree.
...
{
    //const int MAX = 2250; // in us
    const int MAX = 750; // in us
    const int MIN = 0;
    ...
    uint32_t pulseWidth = (MAX - MIN) * deg / 180 + MIN;

    uint32_t period = 20000;
    TIM_Data, Channel, (uint32_t)((TIM_Data->Period - 1) * micros) / TIM_Data->Micros
    return PWMLIB_Write(pwm_id, pulseWidth/period);
}
